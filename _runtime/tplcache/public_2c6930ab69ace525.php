<?php if (!defined('THINK_PATH')) exit();?><div class="pop-message" id='post_message' style="*width:470px">
  <table class="pop table-pop">
    <tbody>
      <tr>
        <td class="textR"><?php echo L('PUBLIC_RECEIVER');?>：</td>
        <td><?php echo W('SearchUser',array('uids'=>$touid,'max'=>1,'editable'=>$editable));?></td>
      </tr>
      <tr>
        <td class="textR"><?php echo L('PUBLIC_INVITE_CONTENT');?>：</td>
        <td>
      ...
      <tr>
        <td>&nbsp;</td>
        <td><a class="btn-green-small" onclick="doPostMessage();" id="postbut" href="javascript:;"><span><?php echo L('PUBLIC_SEND');?></span></a></td>
      </tr>
    </tbody>
  </table>
</div>
...
function doPostMessage() {
    ...
    butstate('close');
    // 发送私信链接
    $.post("<?php echo U('public/Message/doPost');?>", {to:message_to, content:message_content, attach_ids:''}, function(res) {
        if(res.status == 1){
            ui.success("发送成功");
            ui.box.close();
        } else {
